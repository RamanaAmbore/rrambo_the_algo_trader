...
from services.report_ledger_entries import ReportLedgerEntries
from services.report_profit_loss import ReportProfitLoss
from services.report_tradebook import ReportTradebook
...
class ReportUploader:
    ...
    def process_tradebook(self, row: pd.Series, existing_records: set) -> Optional[ReportTradebook]:
        ...
        if not Parms.DOWNLOAD_TRADEBOOK or row["trade_id"] in existing_records:
            return None
    ...
    def process_pnl(row: pd.Series, existing_records: set) -> Optional[ReportProfitLoss]:
        """Process P&L record."""
        if not Parms.DOWNLOAD_PNL or (row["Symbol"], row["ISIN"]) in existing_records:
            return None
    ...
    def process_ledger(row: pd.Series, existing_records: set) -> Optional[ReportLedgerEntries]:
        ...
        if not Parms.DOWNLOAD_LEDGER or (
